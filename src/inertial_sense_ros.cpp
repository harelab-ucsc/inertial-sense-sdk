...
InertialSenseROS::InertialSenseROS(YAML::Node paramNode, bool configFlashParameters) : nh_(), nh_private_("~"), initialized_(false), rtk_connectivity_watchdog_timer_()
{
    if (paramNode.IsDefined())
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Using Yaml file parameters.\n\n");
    }
    else
    {
        load_params_srv();
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Using parameter server.\n\n");
    }
    connect();
    ...
    data_stream_timer_          = nh_.createTimer(rclcpp::Duration(1), configure_data_streams, this); // 2 Hz
    if (diagnostics_.enabled)
    {
        diagnostics_.pub = nh_.advertise<diagnostic_msgs::msg::DiagnosticArray>("diagnostics", 1);
        diagnostics_timer_ = nh_.createTimer(rclcpp::Duration(0.5), &InertialSenseROS::diagnostics_callback, this); // 2 Hz
    }
...
{
    RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Load YAML server");
...
{
    RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Load Param Server");
    printf("\n\nLoading Params");
...
void InertialSenseROS::configure_data_streams(const rclcpp::TimerEvent& event)
{
...
void InertialSenseROS::configure_data_streams(bool startup) // if startup is true each step will be attempted without returning
{
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS1 Pos data stream.");
        SET_CALLBACK(DID_GPS1_POS, gps_pos_t, GPS_pos_callback, 1);
    }
    if (!strobeInStreaming_)
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable strobe in data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable flash config data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable INS1 data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable INS2 data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable INS4 data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable odom INS NED data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable odom INS ECEF data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable odom INS ENU data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable NavSatFix.");
        NavSatFix_.pub = nh_.advertise<sensor_msgs::msg::NavSatFix>("NavSatFix", 1);
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable INS2 States data stream.");
    ...
    if (GPS1_.enabled)
    {
        GPS1_.pub = nh_.advertise<inertial_sense_ros::msg::GPS>(gps1_topic_, 1);
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS1 Vel data stream.");
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS1 RAW data stream.");
            GPS1_raw_.pub = nh_.advertise<inertial_sense_ros::msg::GNSSObsVec>(gps1_topic_ + "/obs", 50);
            GPS1_raw_.pub2 = nh_.advertise<inertial_sense_ros::msg::GNSSEphemeris>(gps1_topic_ + "/eph", 50);
            GPS1_raw_.pub3 = nh_.advertise<inertial_sense_ros::msg::GlonassEphemeris>(gps1_topic_ + "/geph", 50);
            SET_CALLBACK(DID_GPS1_RAW, gps_raw_t, GPS_raw_callback, gps_raw_period_multiple);
            GPS_base_raw_.pub = nh_.advertise<inertial_sense_ros::msg::GlonassEphemeris>("/base_geph", 50);
            GPS_base_raw_.pub2 = nh_.advertise<inertial_sense_ros::msg::GNSSEphemeris>(gps1_topic_ + "/base_eph", 50);
            GPS_base_raw_.pub3 = nh_.advertise<inertial_sense_ros::msg::GlonassEphemeris>(gps1_topic_ + "/base_geph", 50);
            SET_CALLBACK(DID_GPS_BASE_RAW, gps_raw_t, GPS_raw_callback, gps_raw_period_multiple);
            obs_bundle_timer_ = nh_.createTimer(rclcpp::Duration(0.001), InertialSenseROS::GPS_obs_bundle_timer_callback, this);
            if (!startup)
                return;
        }
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS1 Info data stream.");
    ...
    if (GPS2_.enabled)
    {
        GPS2_.pub = nh_.advertise<inertial_sense_ros::msg::GPS>(gps2_topic_, 1);

        // Set up the GPS ROS stream - we always need GPS information for time sync, just don't always need to publish it
        if (!gps2PosStreaming_)
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS2 Pos data stream.");
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS2 Vel data stream.");
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS2 Obs data stream.");
            GPS2_raw_.pub = nh_.advertise<inertial_sense_ros::msg::GNSSObsVec>(gps2_topic_ + "/obs", 50);
            GPS2_raw_.pub2 = nh_.advertise<inertial_sense_ros::msg::GNSSEphemeris>(gps2_topic_ + "/eph", 50);
            GPS2_raw_.pub3 = nh_.advertise<inertial_sense_ros::msg::GlonassEphemeris>(gps2_topic_ + "/geph", 50);
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable GPS Info data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable Mag data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable baro data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable preint IMU data stream.");
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Attempting to enable IMU data stream.");
    ...
    if (!startup)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Inertial Sense ROS data streams successfully enabled.");
        return;
    }
}
...
{
    /// Connect to the uINS
    RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Connecting to serial port \"%s\", at %d baud", port_.c_str(), baudrate_);
    ...
    {
        // Print if Successful
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Connected to uINS %d on \"%s\", at %d baud", IS_.GetDeviceInfo().serialNumber, port_.c_str(), baudrate_);
    }
}
...
void InertialSenseROS::configure_flash_parameters()
{
    ...
    //RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Configuring flash: \nCurrent: %i, \nDesired: %i\n", current_flash_cfg.ioConfig, ioConfig_);

    if (current_flash_cfg.startupNavDtMs != navigation_dt_ms_)
    {
        reboot = true;
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "navigation rate change from %dms to %dms, resetting uINS to make change", current_flash_cfg.startupNavDtMs, navigation_dt_ms_);
    }
    if (current_flash_cfg.ioConfig != ioConfig_)
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "ioConfig change from %x to %x, resetting uINS to make change", current_flash_cfg.ioConfig, ioConfig_);
        reboot = true;
    }
...
void InertialSenseROS::start_rtk_connectivity_watchdog_timer()
{
    ...
    {
        rtk_connectivity_watchdog_timer_ = nh_.createTimer(rclcpp::Duration(rtk_connectivity_watchdog_timer_frequency_), InertialSenseROS::rtk_connectivity_watchdog_timer_callback, this);
    }
...
void InertialSenseROS::rtk_connectivity_watchdog_timer_callback(const rclcpp::TimerEvent &timer_event)
{
    if (rtk_connecting_)
    {
        return;
...
void InertialSenseROS::configure_rtk()
{
    ...
    if (gps1_type_ == "F9P")
    {
        if (RTK_rover_)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: RTK Rover Configured.");
            connect_rtk_client(RTK_correction_protocol_, RTK_server_IP_, RTK_server_port_);
            ...
            RTK_pos_.pub = nh_.advertise<inertial_sense_ros::msg::RTKInfo>("RTK/info", 10);
            RTK_pos_.pub2 = nh_.advertise<inertial_sense_ros::msg::RTKRel>("RTK/rel", 10);
        ...
        if (GNSS_Compass_)
        {
            ...
            RTK_cmp_.pub = nh_.advertise<inertial_sense_ros::msg::RTKInfo>("RTK/info", 10);
            RTK_cmp_.pub2 = nh_.advertise<inertial_sense_ros::msg::RTKRel>("RTK/rel", 10);
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: Dual GNSS (compassing) configured");
        }
        if (RTK_rover_radio_enable_)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: Configured as RTK Rover with radio enabled");
            RTKCfgBits |= RTK_CFG_BITS_ROVER_MODE_RTK_POSITIONING_EXTERNAL;
            SET_CALLBACK(DID_GPS1_RTK_POS_MISC, gps_rtk_misc_t, RTK_Misc_callback, RTK_pos_.period_multiple);
            SET_CALLBACK(DID_GPS1_RTK_POS_REL, gps_rtk_rel_t, RTK_Rel_callback, RTK_pos_.period_multiple);
            RTK_pos_.pub = nh_.advertise<inertial_sense_ros::msg::RTKInfo>("RTK/info", 10);
        ...
        if (RTK_base_USB_)
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: Base Configured.");
    ...
    else
    {
        ...
        {
            RTK_rover_ = false;
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: Configured as dual GNSS (compassing)");

            RTKCfgBits |= RTK_CFG_BITS_ROVER_MODE_RTK_COMPASSING;
            SET_CALLBACK(DID_GPS2_RTK_CMP_MISC, gps_rtk_misc_t, RTK_Misc_callback, RTK_cmp_.period_multiple);
            SET_CALLBACK(DID_GPS2_RTK_CMP_REL, gps_rtk_rel_t, RTK_Rel_callback, RTK_cmp_.period_multiple);
            RTK_pos_.pub = nh_.advertise<inertial_sense_ros::msg::RTKInfo>("RTK_cmp/info", 10);
            RTK_pos_.pub2 = nh_.advertise<inertial_sense_ros::msg::RTKRel>("RTK_cmp/rel", 10);
        }
        ...
        if (RTK_rover_radio_enable_)
        {
            ...
            SET_CALLBACK(DID_GPS1_RTK_POS_MISC, gps_rtk_misc_t, RTK_Misc_callback, RTK_pos_.period_multiple);
            SET_CALLBACK(DID_GPS1_RTK_POS_REL, gps_rtk_rel_t, RTK_Rel_callback, RTK_pos_.period_multiple);
            RTK_pos_.pub = nh_.advertise<inertial_sense_ros::msg::RTKInfo>("RTK_pos/info", 10);
            RTK_pos_.pub2 = nh_.advertise<inertial_sense_ros::msg::RTKRel>("RTK_pos/rel", 10);
        }
        else if (RTK_rover_)
        {
            RTK_base_serial_ = false;
            ...
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: Configured as RTK Rover");
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "InertialSense: Configured as RTK Base");
            if (RTK_base_serial_)
...
{
    if (!flashConfigStreaming_)
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s response received", cISDataMappings::GetDataSetName(DID));
    ...
    refLLA_known = true;
    RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "refLla was set");
}
...
void InertialSenseROS::INS1_callback(eDataIDs DID, const ins_1_t *const msg)
{
    if (!ins1Streaming_)
    {
        ...
        {
            DID_INS_1_.pub = nh_.advertise<inertial_sense_ros::msg::DID_INS1>("DID_INS_1", 1);
        }
...
void InertialSenseROS::INS2_callback(eDataIDs DID, const ins_2_t *const msg)
{
    if (!ins2Streaming_)
    {
        ...
        if (DID_INS_2_.enabled)
            DID_INS_2_.pub = nh_.advertise<inertial_sense_ros::msg::DID_INS2>("DID_INS_2", 1);
    }
...
void InertialSenseROS::INS4_callback(eDataIDs DID, const ins_4_t *const msg)
{
    if (!ins4Streaming_)
    {
        ...
        if (DID_INS_4_.enabled)
            DID_INS_4_.pub = nh_.advertise<inertial_sense_ros::msg::DID_INS4>("DID_INS_4", 1);

        if (odom_ins_ned_.enabled)
            odom_ins_ned_.pub = nh_.advertise<nav_msgs::msg::Odometry>("odom_ins_ned", 1);

        if (odom_ins_enu_.enabled)
            odom_ins_enu_.pub = nh_.advertise<nav_msgs::msg::Odometry>("odom_ins_enu", 1);

        if ( odom_ins_ecef_.enabled)
            odom_ins_ecef_.pub = nh_.advertise<nav_msgs::msg::Odometry>("odom_ins_ecef", 1);
    }

    ins4Streaming_ = true;
    if (!refLLA_known)
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "REFERENCE LLA MUST BE RECEIVED");
        return;
    }
    if (DID_INS_4_.enabled)
    {
    ...
    if (odom_ins_ned_.enabled || odom_ins_enu_.enabled || odom_ins_ecef_.enabled)
    {
        ...
        if (odom_ins_ecef_.enabled)
        {
            ...
            if (publishTf_)
            {
                ...
                br.sendTransform(tf2::StampedTransform(transform_ECEF, rclcpp::Time::now(), "ins_ecef", "ins_base_link_ecef"));
            }
        }
        ...
        if (odom_ins_ned_.enabled)
        {
            ...
            if (publishTf_)
            {
                ...
                br.sendTransform(tf2::StampedTransform(transform_NED, rclcpp::Time::now(), "ins_ned", "ins_base_link_ned"));
            }
        }
        ...
        if (odom_ins_enu_.enabled)
        {
            ...
            if (publishTf_)
            {
                ...
                br.sendTransform(tf2::StampedTransform(transform_ENU, rclcpp::Time::now(), "ins_enu", "ins_base_link_enu"));
            }
        }
    }
}
...
void InertialSenseROS::INL2_states_callback(eDataIDs DID, const inl2_states_t *const msg)
{
    if (!inl2StatesStreaming_)
    {
        ...
        if (INL2_states_.enabled)
            INL2_states_.pub = nh_.advertise<inertial_sense_ros::msg::INL2States>("inl2_states", 1);
    }
...
void InertialSenseROS::strobe_in_time_callback(eDataIDs DID, const strobe_in_time_t *const msg)
{
    ...
    if (strobe_pub_.getTopic().empty())
        strobe_pub_ = nh_.advertise<std_msgs::msg::Header>("strobe_time", 1);
...
void InertialSenseROS::GPS_info_callback(eDataIDs DID, const gps_sat_t *const msg)
{
    if (DID = DID_GPS1_SAT)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s (GPS1 info) response received", cISDataMappings::GetDataSetName(DID));
            if (GPS1_info_.enabled)
                GPS1_info_.pub = nh_.advertise<inertial_sense_ros::msg::GPSInfo>(gps1_topic_ + "/info", 1);
    ...
    if (DID = DID_GPS2_SAT)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s (GPS2 info) response received", cISDataMappings::GetDataSetName(DID));
            if (GPS2_info_.enabled)
                GPS2_info_.pub = nh_.advertise<inertial_sense_ros::msg::GPSInfo>(gps1_topic_ + "/info", 1);
...
void InertialSenseROS::mag_callback(eDataIDs DID, const magnetometer_t *const msg)
{
    if (!magStreaming_)
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s response received", cISDataMappings::GetDataSetName(DID));
        if (mag_.enabled)
            mag_.pub = nh_.advertise<sensor_msgs::msg::MagneticField>("mag", 1);
    }
    magStreaming_ = true;
...
void InertialSenseROS::baro_callback(eDataIDs DID, const barometer_t *const msg)
{
    if (!baroStreaming_)
    {
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s response received", cISDataMappings::GetDataSetName(DID));
        if (baro_.enabled)
            baro_.pub = nh_.advertise<sensor_msgs::msg::FluidPressure>("baro", 1);
    }
...
void InertialSenseROS::preint_IMU_callback(eDataIDs DID, const pimu_t *const msg)
{
    ...
    if (preint_IMU_.enabled)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s response received", cISDataMappings::GetDataSetName(DID));
            preint_IMU_.pub = nh_.advertise<inertial_sense_ros::msg::PreIntIMU>("preint_imu", 1);
        }
    ...
    if (IMU_.enabled)
    {
        if (!imuStreaming_)
        {
            RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "IMU response received");
            IMU_.pub = nh_.advertise<sensor_msgs::msg::Imu>("imu", 1);
        }
        imuStreaming_ = true;
...
void InertialSenseROS::RTK_Rel_callback(eDataIDs DID, const gps_rtk_rel_t *const msg)
{
    ...
    if (abs(GPS_towOffset_) > 0.001)
    {
        ...
        {
            rtk_rel.eGpsNavFixStatus = inertial_sense_ros::msg::RTKRel::GPS_STATUS_FIX_3D;
        }
        else if (fixStatus == GPS_STATUS_FIX_RTK_SINGLE)
        {
            rtk_rel.eGpsNavFixStatus = inertial_sense_ros::msg::RTKRel::GPS_STATUS_FIX_RTK_SINGLE;
        }
        else if (fixStatus == GPS_STATUS_FIX_RTK_FLOAT)
        {
            rtk_rel.eGpsNavFixStatus = inertial_sense_ros::msg::RTKRel::GPS_STATUS_FIX_RTK_FLOAT;
        }
        else if (fixStatus == GPS_STATUS_FIX_RTK_FIX)
        {
            rtk_rel.eGpsNavFixStatus = inertial_sense_ros::msg::RTKRel::GPS_STATUS_FIX_RTK_FIX;
        }
        else if (msg->status & GPS_STATUS_FLAGS_RTK_FIX_AND_HOLD)
        {
            rtk_rel.eGpsNavFixStatus = inertial_sense_ros::msg::RTKRel::GPS_STATUS_FLAGS_RTK_FIX_AND_HOLD;
        }
...
void InertialSenseROS::GPS_raw_callback(eDataIDs DID, const gps_raw_t *const msg)
{
    if (DID == DID_GPS1_RAW)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "%s (GPS raw) response received", cISDataMappings::GetDataSetName(DID));
...
void InertialSenseROS::GPS_obs_callback(eDataIDs DID, const obsd_t *const msg, int nObs)
{
    if (DID == DID_GPS1_RAW)
    {
        ...
        GPS_obs_bundle_timer_callback(rclcpp::TimerEvent());
    }
...
void InertialSenseROS::GPS_obs_bundle_timer_callback(const rclcpp::TimerEvent &e)
{
    if (gps1_obs_Vec_.obs.size() != 0)
    {
...
void InertialSenseROS::diagnostics_callback(const rclcpp::TimerEvent &event)
{
    if (!diagnosticsStreaming_)
        RCLCPP_INFO(rclcpp::get_logger("InertialSenseRos"), "Diagnostics response received");
...
bool InertialSenseROS::set_refLLA_to_value(inertial_sense_ros::srv::refLLAUpdate::Request &req, inertial_sense_ros::srv::refLLAUpdate::Response &res)
{
...
void InertialSenseROS::reset_device()
{
    ...
    RCLCPP_WARN(rclcpp::get_logger("InertialSenseRos"), "Device reset required.\n\nShutting down Node.\n");
    ros::shutdown();
}

bool InertialSenseROS::update_firmware_srv_callback(inertial_sense_ros::srv::FirmwareUpdate::Request &req, inertial_sense_ros::srv::FirmwareUpdate::Response &res)
{
    //   IS_.Close();
